...
```{r}
summary_glm<-summary(glm(NAFLD_status~T2D,data = omitNAFLD))
knitr::kable(summary(glm(NAFLD_status~T2D,data = NAFLDtotal))$coef,digits=3, caption = "Summary table of original NAFLD data")
knitr::kable(summary(glm(NAFLD_status~T2D,data = omitNAFLDtotal))$coef,digits=3, caption = "Summary table of omit original NAFLD data")
knitr::kable(summary(glm(NAFLD_status~T2D,data = NAFLD))$coef,digits=3, caption = "Summary table of adjusted NAFLD data")
knitr::kable(summary_glm$coef,digits=3, caption = "Summary table of omit adjusted NAFLD data")
...
res = lapply(1:boot_rep, function(seed) {
  ...
  Feature.NAFLD[seed+1]<<-coef.l.min1
})
...
print(Feature.NAFLD$feature[Feature.NAFLD$total>0.8*boot_rep])
...
res = lapply(1:boot_rep, function(seed) {
  ...
  fit_cont = glmnet::glmnet(x= X_cont, y = Y_cont, family="binomial", alpha = 1, 
                            standardize = F, lambda = lambda)
  coef.l.min1 <- coef(fit_cont)[-1]
  Feature.T2D[seed+1]<<-coef.l.min1
})
...
print(Feature.T2D$feature[Feature.T2D$total>0.8*boot_rep])
...
boot_rep = 500 # Number of Boot straps
sum.NAFLD<-summary(glm(NAFLD_status~.*., , family="binomial",data = omitNAFLD[,!names(omitNAFLD) %in% "T2D"]))
...
res = lapply(1:boot_rep, function(seed) {
  ...
  # Run LASSO with no covariate control: Continuous Outcome
  ## Convert dataframe into matrix
  X_cont = scale(model.matrix(NAFLD_status~.*.,traindf[,!names(traindf) %in% "T2D"])[,-1])
  Y_cont = traindf$NAFLD_status
  # Find best lambda
  cvfit_cont = glmnet::cv.glmnet(x= X_cont, y = Y_cont,nfolds = 5, family="binomial", 
                                 alpha = 1, standardize = F)
  ...
  fit_cont = glmnet::glmnet(x= X_cont, y = Y_cont, family="binomial", alpha = 1, 
                            standardize = F, lambda = lambda)
  coef.l.min1 <- coef(fit_cont)[-1]
  Feature.NAFLD.interaction[seed+1]<<-coef.l.min1
})
...
print(Feature.NAFLD.interaction$feature[Feature.NAFLD.interaction$total>0.7*boot_rep])
## Variables related to NAFLD: "Triglycerides","HDL_cholesterol",  "Waist_circumference", "Sex1:Triglycerides" 

# Perform Boot straps with LASSO contain interaction for t2d
boot_rep = 500 # Number of Boot straps
sum.T2D <- summary(glm(T2D~.*.,data =omitNAFLD[,!names(omitNAFLD) %in% "NAFLD_status"], family="binomial"))
...
res = lapply(1:boot_rep, function(seed) {
  ...
  # Run LASSO with no covariate control: Continuous Outcome
  ## Convert dataframe into matrix
  X_cont = scale(model.matrix(T2D~.*.,traindf[,!names(traindf) %in% "NAFLD_status"])[,-1])
  Y_cont = traindf$T2D
  ...
  fit_cont = glmnet::glmnet(x= X_cont, y = Y_cont, family="binomial", alpha = 1, 
                            standardize = F, lambda = lambda)
  coef.l.min1 <- coef(fit_cont)[-1]
  Feature.T2D.interaction[seed+1]<<-coef.l.min1
})
...
print(Feature.T2D.interaction$feature[Feature.T2D.interaction$total>0.7*boot_rep])
## Variables related to T2D: "Glycated_haemoglobin__HbA1c_","Glycated_haemoglobin__HbA1c_:Waist_circumference" ,"Glycated_haemoglobin__HbA1c_:Systolic_Blood_Pressure"
```

```{r}
# Functions for performing boot straps and feature sampling for LASSO
...
## Variables related to NAFLD_status: "Glycated_haemoglobin__HbA1c_","Systolic_Blood_Pressure","Triglycerides","Waist_circumference"          
```
...
## Variables related to T2D:"Glycated_haemoglobin__HbA1c_"  "Systolic_Blood_Pressure" "Waist_circumference"
```
```{r}
## Variables related to NAFLD_status:"Glycated_haemoglobin__HbA1c_.Systolic_Blood_Pressure",
##"Glycated_haemoglobin__HbA1c_.Waist_circumference","Systolic_Blood_Pressure.Glycated_haemoglobin__HbA1c_",
##"Systolic_Blood_Pressure.Triglycerides","Systolic_Blood_Pressure.Waist_circumference",
##"Waist_circumference.Glycated_haemoglobin__HbA1c_","Waist_circumference.Systolic_Blood_Pressure" 

## Variables related to T2D:"Glycated_haemoglobin__HbA1c_.Systolic_Blood_Pressure",
##"Glycated_haemoglobin__HbA1c_.Waist_circumference","Systolic_Blood_Pressure.Glycated_haemoglobin__HbA1c_",
##"Waist_circumference.Glycated_haemoglobin__HbA1c_" 
variables.sample.bootstrap<-c("Glycated_haemoglobin__HbA1c_", "Waist_circumference")
variables.sample.bootstrap.interaction<-c("Glycated_haemoglobin__HbA1c_.Systolic_Blood_Pressure","Glycated_haemoglobin__HbA1c_.Waist_circumference","Systolic_Blood_Pressure.Glycated_haemoglobin__HbA1c_","Waist_circumference.Glycated_haemoglobin__HbA1c_")
```
...
A <- cbind2(c("","Method","Greedy Matching","Optimal Matching","1:2 Matching","Full Matching","Subclassification", "IPTW","GLM"),
...
tabletext <- cbind(c("","Method","Greedy Matching","Optimal Matching","1:2 Matching","Full Matching","Subclassification", "IPTW","GLM"), c("","Effect", as.character(round(NAFLD.T2D.MIps.HR.est,3))), c("","SE", as.character(round(NAFLD.T2D.MIps.se,3))),c("","p-value", ifelse(NAFLD.T2D.MIps.pvalue<0.001, "<0.001", round(NAFLD.T2D.MIps.pvalue,3))))
...
```{r}
NAFLD.T2D<-data.frame(method=c(rep("CCA",7),rep("MIte",7),rep("MIps",7)),PSmethod=c(rep(c("Greedy Matching","Optimal Matching","1:2 Matching","Full Matching","Subclassification", "IPTW","GLM"),3)),est=c(round(NAFLD.T2D.HR.est,3),round(NAFLD.T2D.MIte.HR.est,3),round(NAFLD.T2D.MIps.HR.est,3)),log.est=c(round(NAFLD.T2D.logHR.est,3),round(NAFLD.T2D.MIte.logHR.est,3),round(NAFLD.T2D.MIps.logHR.est,3)),se=c(round(NAFLD.T2D.se,3),round(NAFLD.T2D.MIte.se,3),round(NAFLD.T2D.MIps.se,3)),pvalue=c(round(NAFLD.T2D.pvalue,3),round(NAFLD.T2D.MIte.pvalue,3),round(NAFLD.T2D.MIps.pvalue,3)),lowerCI=c(round(NAFLD.T2D.lowerCI,3),round(NAFLD.T2D.MIte.lowerCI,3),round(NAFLD.T2D.MIps.lowerCI,3)),upperCI=c(round(NAFLD.T2D.upperCI,3),round(NAFLD.T2D.MIte.upperCI,3),round(NAFLD.T2D.MIps.upperCI,3)))
